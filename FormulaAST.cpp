...
class Expr {
public:
    using ArgFunc = std::function<double(const Position*)>;

    virtual ~Expr() = default;
    virtual void Print(std::ostream &out) const = 0;
    virtual void DoPrintFormula(std::ostream &out, ExprPrecedence precedence) const = 0;
    virtual double Evaluate(ArgFunc args) const = 0;
...
public:
    ...
    double Evaluate(ArgFunc args) const override {
        double result { };
        switch (type_) {
        case Add:
...
public:
    ...
    double Evaluate(ArgFunc args) const override {
        return (type_ == UnaryMinus) ? operand_->Evaluate(args) * -1.0 : operand_->Evaluate(args);
    }
...
FormulaAST ParseFormulaAST(const std::string &in_str) {
    try {
        ...
        throw FormulaException("Error in ParseFormulaAST");
    }
}
...
double FormulaAST::Execute(ASTImpl::Expr::ArgFunc args) const {
